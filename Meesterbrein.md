...
**Volgende stap:** de resterende admin-portal↔core-overlap (te kiezen met Kevin). Echte strangler-swaps die nog open staan: lokale `storage` → `core.storage`, en de grote `ms_graph` → `core.graph_auth` / `core.graph_api` (de Graph-aanroepen zelf — grootste overlap). Let op: `core.events` is géén duplicaat-swap maar nieuwe dashboard-instrumentatie — een feature-toevoeging, apart aan te pakken. Zelfde patroon (branch → PR → Railway-preview → merge na functionele check). Losse aandachtspunten blijven: secrets roteren (H8.3) en de aantekeningen voor consolidatie hieronder.
...
- **Makelaar nog niet in de agenda-body:** admin-portal geeft `makelaar` nog niet door aan `agenda_format.opmaak_opname` (in Stap 5 leeg gehouden = gedragsneutraal). De core ondersteunt het wél; makelaar daadwerkelijk in de agenda-afspraak tonen is een losse feature voor later (zie ook frictie #9, H6a).
