...
**Volgende stap:** Stap 4 (voorstel) — `ep_online_lookup` in `server.py` wegstrangleren naar `core.ep_online`, volgens hetzelfde patroon (branch → PR → Railway-preview → merge na functionele check). Losse aandachtspunten blijven: secrets roteren (H8.3) en de aantekeningen voor consolidatie hieronder.
...
- **Oude hardcoded BAG-sleutel staat nu in de git-historie (verbrand):** door Stap 3 (verwijderd uit `server.py`) is roteren bij het Kadaster + de env-var bijwerken urgenter geworden. Hoort bij H8.3.
