...
| 6. graph_api | ✅ Klaar | Alle onderdelen af: ✅ agenda, ✅ mail, ✅ onedrive, ✅ todo, ✅ onenote. 51/51 tests groen. Uitgebreid in **v0.5.0** met `onedrive.web_url(pad)` (deelbare OneDrive-link van een map/bestand) voor de dashboard-deeplinks. |
...
- ✅ **Job A — automatisch voorbereiden (PR #16):** een **dagelijkse in-app scheduler** scant de geboekte afspraken (`storage.alle_aankomend()`) en bereidt elke opname die binnen ~48u valt automatisch voor (zelfde flow). **Idempotent** (VBO-dedup via `voorbereid.json`), **robuust** (elke afspraak faalt onafhankelijk), en elke poging wordt **gelogd via core.events**. Ook handmatig te draaien: `POST /api/job-a/run` (hele scan) en `POST /api/dossier/afspraak` (één afspraak). Bron is de afspraak zelf → klant + adres + makelaar al bekend (principe 1).
...
| 4 | Dossier voorbereiden | automatisch (Job A / Voorbereiding, H7.2) | — | ✅ Online: handmatig (knop) én automatisch (Job A, dagelijkse scan ~48u vooraf) — VABI-sjabloon + OneDrive-dossier + onderbouwing + To Do |
...
*(Eerste versie gebouwd in F3, PR #17 — zie H6.)*

- **Pijplijn-bord:** de aankomende opnames met hun dossier-status (de volledige kanban per grove fase/producttype is nog een verfijning).
- **"Hangt op"-overzicht:** welke dossiers wachten en op wie (klant / extern / Kevin) — als `hangt_op`-veld op het dossier (H9.5), afgeleid van de levenscyclus-stap ("wacht-op", principe 2) of handmatig gezet.
...
> **✅ Eerste versie gebouwd (F3, PR #17) — `/dashboard` in de admin-portal.** Toont: (1) **pijplijn** — aankomende opnames met dossier-status (voorbereid / niet / mislukt / verschoven), met knoppen "Dossier voorbereiden" en "Job A nu draaien"; (2) **"hangt op"** — per dossier, afgeleid van de status of handmatig gezet; (3) **technische gezondheid** — BAG/EP (env) + Graph-token + recente mislukte events met tijd + oorzaak; (4) **dossier-detail** — drill-down met gegevens, dossiermap en event-historie. Inclusief **re-prep-detectie** (verschoven afspraak → "opnieuw voorbereiden"). Leest uit `storage` + de voorbereid-registry + `core.events`. Multi-user-bewust (adviseur-filter is later config).
>
> **✅ Verfijnd (F3, PR #18/#19/#20):** (a) **OneDrive-deeplinks** — bij het aanmaken wordt de `webUrl` van de dossiermap vastgelegd (core.graph_api.onedrive `web_url`, **core v0.5.0**) en als klikbare link in het dossier-detail getoond; (b) **filters + tellingen** — klikbare tegels (open / voorbereid / mislukt / verschoven) bovenaan de pijplijn; (c) **ververs-knop + lichte auto-refresh** (elke 60s aan/uit) zodat het dashboard geen momentopname is; (d) **technische gezondheid opgeschoond** — alleen nog **openstaande** fouten (een bron die later alsnog lukte verdwijnt automatisch); (e) **dagcyclus-samenvatting** — kaart met "X voorbereid, Y mislukt" uit de laatste Job A-run (events). **Nog te verfijnen:** het volledige kanban-bord per grove fase/producttype (wacht op latere levenscyclus-stappen die status-events schrijven).
...
- **Job A (voorbereiden)** — ✅ **gebouwd (F3, PR #16):** een dagelijkse in-app scheduler scant de geboekte afspraken voor de komende ~48 uur en bereidt per opname automatisch het dossier voor (VABI-sjabloon + OneDrive-map + onderbouwing + To Do), idempotent (VBO-dedup) en robuust per afspraak, met logging via core.events. Ook handmatig te draaien (`POST /api/job-a/run`, `POST /api/dossier/afspraak`). *(Planning-keuze: in-app scheduler i.p.v. Railway-cron, omdat admin-portal één single-instance service is.)* **✅ Verfijnd (PR #19):** (1) **weekend-slim venster** — op vrijdag wordt het venster verlengd t/m eind maandag, zodat opnames van za/zo/ma niet tussen wal en schip vallen (de scan draait dan pas maandag weer); (2) **bewijsdocument-cache** — de PDOK-luchtfoto wordt per VBO gecachet, zodat opnieuw voorbereiden niet opnieuw ophaalt; (3) **optionele samenvattingsmail** — na elke dagcyclus een kort mailtje ("X voorbereid, Y mislukt") naar het bedrijfsadres, **standaard uit** (instelbaar). De **draaitijd, het venster, actief-aan/uit en de mail** zijn nu instelbaar op `/instellingen` (zie hieronder).
...
| Te doen | Bewijs-PDF via Playwright (onbewezen op Railway); VABI-berekening blijft handwerk bij Kevin (cloud rekent niet); OneNote actief zodra geconfigureerd; daarna aangestuurd door Job A. |
