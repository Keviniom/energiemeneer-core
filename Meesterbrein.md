...
**Volgende stap:** Stap 5 (te kiezen met Kevin) — de resterende core-overlap in de admin-portal wegstrangleren. Kandidaten: lokale `ms_graph` → `core.graph_auth` / `core.graph_api` (grootste overlap), de agenda-titelopmaak → `core.agenda_format`, en logging → `core.events`. Zelfde patroon (branch → PR → Railway-preview → merge na functionele check). Losse aandachtspunten blijven: secrets roteren (H8.3) en de aantekeningen voor consolidatie hieronder.
