...
@pytest.fixture
def _fake_bag(monkeypatch):
    monkeypatch.setenv("USE_FAKE_CLIENTS", "1")
    monkeypatch.delenv(bag._ENV_KEY_BAG, raising=False)  # geen key nodig in fake
...
def test_zoek_adres_fake_geeft_fixture_zonder_netwerk(_fake_bag):
    r = bag.zoek_adres("2548 bv", 8)
    assert r["postcode"] == "2548BV"  # genormaliseerd en ge-echo'd
    assert r["huisnummer"] == 8
    assert r["vbo_id"] and r["pand_ids"] and r["bouwjaar"]


def test_zoek_adres_fake_eist_nog_steeds_input(_fake_bag):
    with pytest.raises(ValueError, match="Postcode"):
        bag.zoek_adres("", 8)


def test_vrij_zoeken_fake_geeft_suggestie(_fake_bag):
    out = bag.vrij_zoeken("Teststraat 8")
    assert len(out) == 1
    assert out[0]["vbo_id"] and out[0]["pand_id"]


def test_vrij_zoeken_fake_lege_invoer_blijft_leeg(_fake_bag):
    assert bag.vrij_zoeken("   ") == []
